...
void EMPI_Monitor_end (int *rank, int *size, int iter, int maxiter, int *count, int *disp, int **vcount, int **vdisp, int *fc, char *argv[], char *bin) {
    ...
    #if (EMPI_DBGMODE > EMPI_DBG_QUIET)
        fprintf (stdout, "\n*** DEBUG_MSG::enter::EMPI_Monitor_end in <%s> ***\n", __FILE__);
		fflush(NULL);
    #endif
    ...
    EMPI_host_type * energy_hostlist = EMPI_GLOBAL_hostlist;

    //Iteration end
    EMPI_Iteration_end (&rtime, &ptime, &flops, &hwpc_1, &hwpc_2, &ctime,&iotime);
	
    //if(*rank == EMPI_root && (iter % EMPI_GLOBAL_niter) == 0) printf("l940\t it = %d \t %lld\t %lld\t %lld\n", iter, flops, hwpc_1, rtime);
    //printf("[DEBUG] monitornig values for iteration %d: %lld %lld %lld\n", flops, hwpc_1, hwpc_2);
    //printf("[DEBUG] Energy values for iteration %d: %lf %lf %lf %lf\n", iter, EMPI_GLOBAL_ENERGY_values[0], EMPI_GLOBAL_ENERGY_values_power[0], EMPI_GLOBAL_ENERGY_values[1], EMPI_GLOBAL_ENERGY_values_power[1]);

    //printf("1. flops is %lld\n", flops);
    ...
    if ((*rank == EMPI_root)&&(EMPI_GLOBAL_initnc == EMPI_TRUE)) {
        ...
        //inicio de ejecucion, primera configuracion
        EMPI_GLOBAL_cost[EMPI_GLOBAL_nc].timestep = EMPI_GLOBAL_iterative_ini;
    ...
    if (EMPI_GLOBAL_mpolicy == EMPI_MONITORDBG) {
        ...
        if ((iter > 0)&&((iter % EMPI_GLOBAL_niter) == 0)) {
            ...
            if (*rank == EMPI_root) {
                ...
                EMPI_GLOBAL_cost[EMPI_GLOBAL_nc].timestep = MPI_Wtime();
       ...
       if ((iter > 0)&&((iter % EMPI_GLOBAL_niter) == 0)) {
            ...
            if (*rank == EMPI_root) {
                ...
                EMPI_GLOBAL_nc ++; // Stores one entry (array) per sample iteration
            }
            ...
            if (EMPI_GLOBAL_Data->stype == EMPI_SPARSE) {
                ...
                if (EMPI_GLOBAL_allocation == EMPI_NULL) EMPI_GLOBAL_allocation = EMPI_NNZ;
			...
			if(smonitor[0].corebinding==1){
				...
				while (fgets(path, 1000, fp) != NULL) {
					printf("%s[%d]: before running on CPU %s", processor_name, *rank, path);
				}
				pclose(fp);
				...
				if (EMPI_GLOBAL_corebindlist[*rank][0]>0){
					CPU_ZERO(&mask);
					for(n=0;n<EMPI_GLOBAL_corebindlist[*rank][0];n++){						
				...
				fp = popen(shell_cmd, "r");
				while (fgets(path, 1000, fp) != NULL) {
					printf("%s[%d]: after running on CPU %s", processor_name, *rank, path);
				}
				pclose(fp);
    ...
    //Last iteration
    if ((*rank == EMPI_root)&&(iter == (maxiter-1))) {
        ...
        //registrar configuracion para costes
        EMPI_GLOBAL_cost[EMPI_GLOBAL_nc].timestep = EMPI_GLOBAL_iterative_end;
        ...
        for (n = 0; n < EMPI_GLOBAL_nc; n ++) {

            class = EMPI_GLOBAL_system_classes;

            for (c = 0; c < EMPI_GLOBAL_nhclasses; c ++) {
    ...
    //debug
    #if (EMPI_DBGMODE > EMPI_DBG_QUIET)
        fprintf (stdout, "\n*** DEBUG_MSG::exit::EMPI_Monitor_end in <%s> ***\n", __FILE__);
    #endif
}
...
static void EMPI_Monitor_efficiency (int *rank, int *size, int iter, int maxiter, int count, int disp, EMPI_Monitor_type *smonitor, int *fc, char *argv[], char *bin) {
    ...
    if ((lbalance == EMPI_BALANCED)||(lbalance == EMPI_NULL)) {
        ...
        if ((*rank == EMPI_root)&&(sflops > 0)) {
            ...
            if (status == EMPI_NON_DEDICATED) printf ("WARNING: detected non-dedicated at %i\n", iter);
...
static void EMPI_Monitor_efficiency_irregular (int *rank, int *size, int iter, int maxiter, int count, int disp, EMPI_Monitor_type *smonitor, int *fc, char *argv[], char *bin) {
    ...
    if ((*rank == EMPI_root)&&(sdflops > 0)) {
        ...
        printf ("%i-%i - rtime %lf (comp %lf comm %lf) tflops %lld otime %lf\n", iter-EMPI_GLOBAL_niter, iter, treal_consum-otime, (((double)((double)sum_tflops)/(double)sdflops) * 1.0E-6), sum_ctime / *size, sum_tflops, otime);
        ...
        printf ("%i-%i - stime %lf (comp %lf comm %lf) sflops %.0lf - resttime %lf percentage %lf - cum_time %lf\n", iter, iter+EMPI_GLOBAL_niter, stime, comp_cost, comm_cost, EMPI_GLOBAL_profile_flops[iter/EMPI_GLOBAL_niter] * flop_factor, resttime, EMPI_GLOBAL_percentage, EMPI_GLOBAL_tcomp + EMPI_GLOBAL_tcomm + EMPI_GLOBAL_tover);
        //Adding procs - stime (estimated time)
        //if (stime > resttime) {
        if ((stime > resttime)&&(stime > (resttime * (1 + EMPI_GLOBAL_obj_texec_threshold)))) {
            ...
            if (EMPI_GLOBAL_Adaptability_policy == EMPI_ADAPTABILITY_LP) {
                ...
                for (p = 1, rprocs = 0; p <= (EMPI_GLOBAL_maxprocs - *size); p ++) {
                    ...
                    //Se selecciona la configuracion con el menor numero de procesos dentro de un threshold
                    if ((rprocs == 0)&&(diff < (resttime * EMPI_GLOBAL_obj_texec_threshold))) {
                        printf ("Estimacion computo %lf estimacion communicaciones %lf overheads %lf - diff %lf maxdiff %lf\n", ptime, comm_cost, spawn_cost + rdata_cost, diff, resttime * EMPI_GLOBAL_obj_texec_threshold);
                        EMPI_GLOBAL_comm_prev = comm_cost;
        ...
        //} else if (stime < resttime) {
            ...
            if (EMPI_GLOBAL_Adaptability_policy == EMPI_ADAPTABILITY_LP) {
                ...
                for (p = 1, rprocs = 0, realflops = 0; p <= (*size - EMPI_GLOBAL_minprocs); p ++) {
                    ...
                    //Se selecciona la configuracion con mayor numero de procesos y que su desviacion no supere el threshold
                    if ((p > rprocs)&&(diff < (resttime * EMPI_GLOBAL_obj_texec_threshold))) {
                        printf ("Estimacion computo %lf estimacion communicaciones %lf overheads %lf - diff %lf maxdiff %lf\n", ptime, comm_cost, remove_cost + rdata_cost, diff, resttime * EMPI_GLOBAL_obj_texec_threshold);
                        EMPI_GLOBAL_comm_prev = comm_cost;
        ...
        if ((stime > resttime)&&(stime > (resttime * (1 + EMPI_GLOBAL_obj_texec_threshold)))) {
            ...
            if (EMPI_GLOBAL_Adaptability_policy == EMPI_ADAPTABILITY_LP) {
                ...
                for (p = 1, rprocs = 0, mincost = -1; p <= (EMPI_GLOBAL_maxprocs - *size); p ++) {
                    ...
                    //Se selecciona la configuracion con el menor coste dentro de un threshold
                    if (((mincost == -1)||(pscost < mincost))&&(diff < (resttime * EMPI_GLOBAL_obj_texec_threshold))) {
                        printf ("Estimacion computo %lf estimacion communicaciones %lf - diff %lf maxdiff %lf\n", ptime, comm_cost, diff, resttime * EMPI_GLOBAL_obj_texec_threshold);
                        EMPI_GLOBAL_comm_prev = comm_cost;
...
static void EMPI_Monitor_malleability_conditional (int *rank, int *size, int iter, int maxiter, int count, int disp, EMPI_Monitor_type *smonitor, int *fc, char *argv[], char *bin) {
    ...
    if (EMPI_GLOBAL_listrm[EMPI_GLOBAL_nextrm] == iter) {
        ...
        //Bcast newsize and mflops
		if(*rank == EMPI_root) printf("\n ===> %d %d", reconfig[0], reconfig[1]);
        ...
        switch (comms->mpi_op) {
            ...
            case MPI_SEND:
                ...
                if (comms->datasize > 1) {
                    ...
                    if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_SEND is %f\n", aux_cost);
                    *cost += aux_cost;
            ...
            case MPI_RECV:
                ...
                if (comms->datasize > 1){
                    ...
                    //MPI_Send communication cost
                    aux_cost = ((EMPI_GLOBAL_alpha + ((comms->datasize * ratio_datasize * btype * 8) * EMPI_GLOBAL_beta)) * ratio_nmess);
                    if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_RECV is %f\n", aux_cost);
                    *cost += aux_cost;
            ...
            case MPI_REDUCE:
                ...
                if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_REDUCE is %f\n", aux_cost);
                *cost += aux_cost;
            ...
            case MPI_BCAST:
                ...
                *cost += aux_cost;
                if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_BCAST is %f\n", aux_cost);
            ...
            case MPI_GATHER:
                ...
                if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_GATHER is %f\n", aux_cost);
                *cost += aux_cost;
            ...
            case MPI_SCATTER:
                aux_cost = log2(newsize) * EMPI_GLOBAL_alpha + (comms->datasize * btype * 8) * ((newsize - 1)/newsize) *  EMPI_GLOBAL_beta;
                if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_SCATTER is %f\n", aux_cost);
                *cost += aux_cost;
            ...
            case MPI_ALLTOALL:
                ...
                if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_ALLTOALL is %f\n", aux_cost);
                *cost += aux_cost;
            ...
            case MPI_REDUCE_SCATTER:
                if(fmod(log2(newsize), 1.0) == 0){
                    ...
                    if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_REDUCE_SCATTER is %f\n", aux_cost);
                    *cost += aux_cost;
                } else {
            ...
            case MPI_ALLREDUCE:
                ...
                */
                if(fmod(log2(newsize), 1.0) == 0){
                    ...
                    if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_ALLREDUCE is %f\n", aux_cost);
                    *cost += aux_cost;
                } else {
            ...
            case MPI_ALLGATHERV:
                ...
                */
                ...
                if(fmod(log2(newsize), 1.0) == 0){
                    ...
                    if(EMPI_GLOBAL_debug_comms)printf("Estimated communication cost for MPI_ALLGATHER is %f\n", aux_cost);
                    *cost += aux_cost;
                } else {
            ...
            default:
                printf("Communication cost is not defined for the current operation %d\n", comms->mpi_op);
                break;
        }
...
static void EMPI_Monitor_spawn (int *rank, int *size, int nprocs, int count, int disp, char *argv[], char *bin, EMPI_Monitor_type *smonitor, int *hostid, int* fc) {
    ...
    if (*rank == EMPI_root) {
        ...
        printf ("Spawn %i at %f\n", nprocs, MPI_Wtime()-EMPI_GLOBAL_iterative_ini);
...
static void EMPI_Monitor_remove (int *rank, int *size, int nprocs, int count, int disp, EMPI_Monitor_type *smonitor, int* rremvs, int* fc) {
    ...
    if (*rank == EMPI_root) printf ("Remove %i at %f\n", nprocs, MPI_Wtime()-EMPI_GLOBAL_iterative_ini);
    ...
    for (arank = *rank, n = 0; n < nprocs; n ++) {
        ...
        if (*rank == EMPI_root) {
            ...
            printf ("Process [%i] removed from %s\n", rremvs[n], hostlist->hostname);
        }
