...
long int    EMPI_IOBLOCK = 0;
int         EMPI_IOWAIT = 0;
double      EMPI_IOBW = 0;
double      EMPI_DESIRED_BW = 0;
double      EMPI_SCALE_BW = 1;
...
int main (int argc, char *argv[])
{
    ...
    // IO PARAMETERS
    EMPI_IOBLOCK = atol(argv[8]);
    EMPI_IOWAIT = atoi(argv[9]);
    EMPI_DESIRED_BW  = atof(argv[10]);
    ...
    if(rank==0) printf(" [%d] Configuration: EMPI_IOBLOCK : %ld \t  EMPI_IOWAIT : %ld \n\n",rank,EMPI_IOBLOCK,(long int)EMPI_IOWAIT);
...
void jacobi (int dim, int *rank, int *size, double *A, double *b, double *x, double *x_old, int it, int itmax, double diff_limit, int cpu_i, int com_i, int io_i, int app_id, char *argv[])
{
    ...
    //strcpy(file, "/lustre/dexposit/test");
    strcpy(file, "/mnt/gluster/desingh");
    ...
    for (; it < itmax; it ++) {
        ...
        if(it%100==0 && io_i>0){
            ...
            double iot1, iot2,datasize;
            int local_size;
            MPI_Barrier (EMPI_COMM_WORLD);
            iot1=MPI_Wtime();            
            for (m=0;m<io_i;m++){        
                MPI_File_write(fh, A, count*dim, MPI_DOUBLE, MPI_STATUS_IGNORE);
            }
            MPI_File_close(&fh);
            MPI_Barrier (EMPI_COMM_WORLD);
            iot2=MPI_Wtime();
           
            MPI_Type_size(MPI_DOUBLE,&local_size);
